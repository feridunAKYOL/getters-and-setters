...
const registerDirectory = function(dirPath, oldPath) {
	...
	return {
		path: dirPath.split(oldPath).join('').split('/').join('') + '/',
		...
		? value === -1
			...
			: value === 0
				...
				: value === 1
					...
					: value === 2
						? 'fail'
						: value === 3 ? 'warning' : value === 4 ? 'error' : value === 5 ? 'syntaxError' : 'unknown status'
		: value;
...
const evaluateFile = (path) => {
	...
	const loopProtectedCode = rawCode.replace(/for *\(.*\{|while *\(.*\{|do *\{/g, (loopHead) => {
		number_of_loops++;
...
const evaluateDirectory = (toReport, path) => {
	...
	const dirs = toReport.dirs ? toReport.dirs.map((dir) => evaluateDirectory(dir, path + dir.path)) : toReport.dirs;

	const statusOf = (entries) =>
		entries ? entries.reduce((stat, entry) => (entry.status > stat ? entry.status : stat), -1) : -1;
...
const generateFileSectionMd = (fileReport) => {
	...
	const source = fileReport.source ? '```js\n' + fileReport.source + '\n```\n\n' : '';

	const topLink = '[TOP](#' + CONFIG.repo + ')';

	return divider + '\n\n' + header + '\n\n' + sourceLink + '\n' + report + source + topLink + '\n';
};
	...
	const fileList = report.files
		? report.files
				.map((file) => {
					...
					return `* [${relPath}](#${localHref}---${interpret('status', file.status)
						.split(' ')
...
const cleanedTopLevelREVIEWSource = topLevelREVIEWSource.split('* [../REVIEW.md](../REVIEW.md)\n').join('');
